...
struct AuctionUploadView: View {
    ...
    // Basic Info
    @State private var carTitle = ""
    @State private var subtitle = ""
    @State private var lotNumber = ""
    @State private var rating = 4.8
    @State private var startingBid = ""
    @State private var auctionEndDate = Date().addingTimeInterval(7 * 24 * 3600) // 7 days from now
    @State private var auctionStatus: AuctionStatus = .upcoming
    ...
    enum AuctionStatus: String, CaseIterable {
            ...
            VStack(spacing: 8) {
                HStack(spacing: 4) {
                    ForEach(0..<steps.count, id: \.self) { index in
                        HStack(spacing: 0) {
                            Circle()
                                .fill(index <= currentStep ? Color.theme.primaryColor : Color.theme.onSurfaceColor.opacity(0.3))
        ...
        VStack(alignment: .leading, spacing: 24) {
            ...
            VStack(spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    DatePicker("", selection: $auctionEndDate, displayedComponents: [.date, .hourAndMinute])
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Picker("Status", selection: $auctionStatus) {
        ...
        VStack(alignment: .leading, spacing: 24) {
            ...
            VStack(spacing: 20) {
                ReviewCard(title: "Basic Information") {
                    VStack(alignment: .leading, spacing: 8) {
                        Text("\(carTitle) - \(subtitle)")
                            .font(.custom("Exo2-SemiBold", size: 16))
                        Text("Lot #\(lotNumber) • Rating: \(String(format: "%.1f", rating))")
                            .font(.custom("Exo2-Regular", size: 14))
                            .foregroundColor(.theme.onSurfaceColor.opacity(0.7))
                        Text("Starting Bid: $\(startingBid)")
                ...
                ReviewCard(title: "Images") {
                    Text("\(selectedImages.count) photos uploaded")
                        .font(.custom("Exo2-Regular", size: 14))
                        .foregroundColor(.theme.onSurfaceColor.opacity(0.7))
                }
    ...
    private var navigationButtons: some View {
        HStack(spacing: 16) {
            ...
            Button(action: {
                ...
                Text(currentStep == steps.count - 1 ? "Submit Auction" : "Next")
                    .font(.custom("Exo2-SemiBold", size: 16))
                    .foregroundColor(.white)
