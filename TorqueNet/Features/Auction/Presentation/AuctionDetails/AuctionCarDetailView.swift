    ...
    var body: some View {
        ZStack {
            ...
            if auctionDetailViewModel.auctionDetailsUiState.timeRemaining  > 0 {
                ...
                // Change auction status and adjust its time if the status is changed to live
            }
...
/*
         ...
         VStack(spacing: 20) {
             HStack {
                 ...
                 VStack(alignment: .trailing, spacing: 8) {
                     ...
                     HStack(spacing: 4) {
                         Circle()
                             .fill(auctionDetailViewModel.auctionDetailsUiState.timeRemaining > 0 ? .green : .red)
                             .frame(width: 8, height: 8)
                         Text(auctionDetailViewModel.auctionDetailsUiState.timeRemaining > 0 ? "Live Bidding" : "Auction Ended")
                             .font(.custom("Exo2-Medium", size: 12))
                             .foregroundColor(.theme.onSurfaceColor.opacity(0.7))
                     }
                 }
 ...
 // MARK: - Auction Status Management Sheet
 struct AuctionStatusManagementSheet: View {
     ...
     @State private var customDate: Date = Date().addingTimeInterval(86400) // 24 hours from now
         ...
         NavigationView {
             ScrollView {
                 VStack(spacing: 24) {
                     ...
                     // Time Extension (only shown when status is "live")
